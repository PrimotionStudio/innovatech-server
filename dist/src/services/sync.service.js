...
const RESOURCE_TYPES = ["course", "cbt", "model", "app"];
export const GetSyncManifest = async (c) => {
    ...
    const installedVersion = (resourceType, resourceId) => installed.find((s) => s.resourceType === resourceType && s.resourceId === resourceId)?.installedVersion ?? null;
    return c.json({
 ...
 */
export const ReportSyncState = async (c) => {
    const device = c.get("device");
    const { entries } = SyncReportSchema.parse(await c.req.json());
    const rejected = entries.filter((e) => !RESOURCE_TYPES.includes(e.resourceType));
    if (rejected.length > 0) {
        return c.json({
            message: `Unknown resource type: ${rejected[0].resourceType}`,
        }, 400);
    }
    await prisma.$transaction(entries.map((entry) => prisma.deviceSyncState.upsert({
        where: {
