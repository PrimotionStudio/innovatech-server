...
export const ReportActivity = async (c: Context) => {
  ...
  if (report.profile && Object.keys(report.profile).some((k) => report.profile![k as keyof typeof report.profile] !== undefined)) {
 ...
 */
export const GetInsightsSummary = async (c: Context) => {
  const now = new Date();
  ...
  const [
    ...
    prisma.$queryRaw<
      { devices: bigint }[]
    >`SELECT COUNT(DISTINCT "deviceId") AS devices FROM "DeviceSession" WHERE "startedAt" >= ${since24h}`,
    prisma.$queryRaw<
      {
        day: Date;
        seconds: bigint | null;
  ...
  const toNumber = (v: bigint | number | null | undefined) =>
    v == null ? 0 : Number(v);

  const subjectMap = new Map<
    string,
    { views: number; seconds: number }
  >();
  ...
  const dailyMap = new Map<string, { seconds: number; sessions: number; devices: number }>();
  ...
  const series: { date: string; studyMinutes: number; sessions: number; activeDevices: number }[] =
    [];
